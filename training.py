...
def fit_lstm(model, loader, n_epochs, n_neurons, learning_rate, optimizer, opt_scheduler,writer,selecteddata,trav_mean,trav_std,road):
    ...
    for epoch in range(n_epochs):  
        for phase in ['train','validation','test']:
            ...
            if phase == 'train':
                train_mse = mean_squared_error(actual, predicted,squared=True)
                train_rmse = mean_squared_error(actual, predicted,squared=False)
                train_mape = mean_absolute_percentage_error(actual, predicted)
                train_mae = mean_absolute_error(actual, predicted)                
                train_smape = utils.symmetric_mean_absolute_error(actual, predicted)
                print(f'| {phase} mse: {train_mse:.5f} | rmse: {train_rmse:.5f} | mape: {train_mape:.5f} | smape: {train_smape:.5f} | mae: {train_mae:.5f}')
                writer.add_scalar('train/rmse', train_rmse, epoch)
                writer.add_scalar('train/mae', train_mae, epoch)
                writer.add_scalar('train/mape', train_mape, epoch)
                #wandb.log({"Epoch mse train loss (lstm)": train_mse, "Epoch rmse train loss (lstm)": train_rmse, "Epoch mape train loss (lstm)": train_mape, "Epoch smape train loss (lstm)": train_smape,"Epoch mae train loss (lstm)": train_mae}, step=epoch, commit=False)
            elif phase == 'validation':
                val_mse = mean_squared_error(actual, predicted,squared=True)
                val_rmse = mean_squared_error(actual, predicted,squared=False)
                val_mape = mean_absolute_percentage_error(actual, predicted)
                val_mae = mean_absolute_error(actual, predicted) 
                val_smape = utils.symmetric_mean_absolute_error(actual, predicted)
                print(f'| {phase} mse: {val_mse:.5f} | rmse: {val_rmse:.5f} | mape: {val_mape:.5f} | smape: {val_smape:.5f} | mae: {val_mae:.5f}')
                ...
                writer.add_scalar('val/smape', val_smape, epoch)                
                if val_mae < best_mae:
                    best_mae = val_mae
